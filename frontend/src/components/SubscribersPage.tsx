...
export function SubscribersPage({ session }: { session: Session }) {
  ...
  const load = useCallback(() => apiRequest<any>(`/customers?search=${encodeURIComponent(search)}`, {}, session.token).then(response => { setRows(response.data.data); setTotal(response.data.total) }).catch(exception => setError(exception instanceof Error ? exception.message : 'Unable to load subscribers.')), [search, session.token])
  useEffect(() => { void load() }, [load])
  const close = () => { setModal(null); setModalError('') }
  const submit = async (values: Record<string, string>) => {
    if (!modal || modal.mode === 'view') return
    setLoading(true); setModalError('')
    try { await apiRequest(modal.mode === 'create' ? '/customers' : `/customers/${modal.row?.public_id}`, { method: modal.mode === 'create' ? 'POST' : 'PUT', body: JSON.stringify(values) }, session.token); close(); await load() }
    catch (exception) { setModalError(exception instanceof Error ? exception.message : 'Unable to save subscriber.') }
    finally { setLoading(false) }
  }
  const archive = async (row: Row) => { try { await apiRequest(`/customers/${row.public_id}`, { method: 'DELETE' }, session.token); await load() } catch (exception) { setError(exception instanceof Error ? exception.message : 'Unable to archive subscriber.') } }
