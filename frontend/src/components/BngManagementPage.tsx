...
type OltOption = { public_id: string; name: string; vendor: string }
type VlanSync = { public_id: string; vlan_mode: 'normal' | 'qinq'; enabled: boolean; olt?: OltOption | null; interfaces?: Array<{ interface_name: string; status: string; last_error?: string | null }> }
type VlanSyncForm = { olt_id: string; vlan_mode: 'normal' | 'qinq'; enabled: boolean }
type ManagementTab = 'ssh' | BngServiceTab | 'bng_settings'
...
type RadiusServer = { public_id: string; name: string; server_address: string; secret?: string; database_name: string; database_username: string; auth_port: number; accounting_port: number; sync_subscribers?: boolean; status?: string | null; notes?: string | null }
type RadiusForm = { name: string; server_address: string; secret: string; database_name: string; database_username: string; database_password: string; auth_port: string; accounting_port: string; sync_subscribers: boolean; notes: string }
...
const emptyRadius = (): RadiusForm => ({ name: '', server_address: '', secret: '', database_name: '', database_username: '', database_password: '', auth_port: '1812', accounting_port: '1813', sync_subscribers: false, notes: '' })
const emptyVlanSync = (): VlanSyncForm => ({ olt_id: '', vlan_mode: 'normal', enabled: true })
...
export function BngManagementPage({ token, publicId, onBack }: { token: string; publicId: string; onBack: () => void }) {
  ...
  const [vlanSyncRows, setVlanSyncRows] = useState<VlanSync[]>([])
  const [vlanSyncLoading, setVlanSyncLoading] = useState(false)
  const [vlanSyncSaving, setVlanSyncSaving] = useState(false)
  const [vlanSyncModalOpen, setVlanSyncModalOpen] = useState(false)
  const [vlanSyncForm, setVlanSyncForm] = useState<VlanSyncForm>(emptyVlanSync())
  const [oltOptions, setOltOptions] = useState<OltOption[]>([])
  ...
  const loadVlanSyncs = async () => {
    setVlanSyncLoading(true)
    try { const response = await apiRequest<{ data: VlanSync[] }>(`/bngs/${publicId}/vlan-syncs`, {}, token); setVlanSyncRows(response.data || []) }
    catch (exception) { notify.error(getErrorMessage(exception, 'Unable to load BNG VLAN sync settings.')) }
    finally { setVlanSyncLoading(false) }
  }

  const loadOltOptions = async () => {
    try {
      const response = await apiRequest<{ data: { data: OltOption[] } }>('/olts?per_page=100', {}, token)
      setOltOptions(response.data?.data || [])
    } catch (exception) { notify.error(getErrorMessage(exception, 'Unable to load OLTs.')) }
  }
  ...
  const saveInterfaces = async () => {
    ...
    try {
      const response = await apiRequest<{ data: Bng }>(`/bngs/${publicId}/interfaces`, { method: 'PATCH', body: JSON.stringify({ parent_interface: parentInterface.trim(), egress_interface: egressInterface.trim() }) }, token)
      setBng(response.data)
      notify.success('BNG interfaces saved.')
    }
  ...
  const openVlanSyncModal = () => { setVlanSyncForm(emptyVlanSync()); setVlanSyncModalOpen(true); void loadOltOptions() }
  const saveVlanSync = async () => {
    setVlanSyncSaving(true)
    try {
      await notify.promise(apiRequest(`/bngs/${publicId}/vlan-syncs`, { method: 'POST', body: JSON.stringify(vlanSyncForm) }, token), { loading: 'Saving VLAN sync...', success: 'VLAN sync added.', error: 'Unable to save VLAN sync.' })
      setVlanSyncModalOpen(false); setVlanSyncForm(emptyVlanSync()); await loadVlanSyncs()
    } catch (exception) { notify.error(getErrorMessage(exception, 'Unable to save VLAN sync.')) }
    finally { setVlanSyncSaving(false) }
  }
  const removeVlanSync = async (sync: VlanSync) => {
    try { await notify.promise(apiRequest(`/bngs/${publicId}/vlan-syncs/${sync.public_id}`, { method: 'DELETE' }, token), { loading: 'Removing VLAN sync...', success: 'VLAN sync removed.', error: 'Unable to remove VLAN sync.' }); await loadVlanSyncs() }
    catch (exception) { notify.error(getErrorMessage(exception, 'Unable to remove VLAN sync.')) }
  }
  const toggleVlanSync = async (sync: VlanSync) => {
    try { await notify.promise(apiRequest(`/bngs/${publicId}/vlan-syncs/${sync.public_id}`, { method: 'PATCH', body: JSON.stringify({ enabled: !sync.enabled }) }, token), { loading: sync.enabled ? 'Disabling VLAN sync...' : 'Synchronizing existing VLANs...', success: sync.enabled ? 'VLAN sync disabled.' : 'VLAN sync enabled and reconciled.', error: 'Unable to update VLAN sync.' }); await loadVlanSyncs() }
    catch (exception) { notify.error(getErrorMessage(exception, 'Unable to update VLAN sync.')) }
  }
  ...
  const active = ['connecting', 'connected', 'disconnected'].includes(sessionStatus)
  const selectTab = (tab: ManagementTab) => { setActiveTab(tab); if (tab === 'accel_ppp' && !accelLoaded && sessionStatus === 'connected') void loadAccel(); if (tab === 'cgnat') void loadCgnat(); if (tab === 'radius') void loadRadius(); if (tab === 'bng_settings') void loadVlanSyncs() }
  ...
  return <div className="flex flex-col gap-5"><div className="billing-page-heading flex flex-wrap items-end justify-between gap-4"><div><p className="billing-eyebrow">Network / BNG management</p><h1 className="mt-2 text-xl font-semibold">{bng.name}</h1><p className="mt-1 text-sm text-muted-foreground">{bng.vendor} {bng.model || 'BNG'} · {bng.management_endpoint || 'No endpoint'} · {bng.preferred_transport || 'SSH'}</p></div><div className="flex items-center gap-2"><Badge variant="outline">{sessionStatus}</Badge><Button variant="outline" onClick={onBack}><ArrowLeftIcon data-icon="inline-start" />Back to BNGs</Button></div></div><Card className="billing-records overflow-hidden"><CardHeader><CardTitle>BNG management</CardTitle><CardDescription>{driver.label}. Manage the persistent SSH session and broadband access services.</CardDescription></CardHeader><CardContent><div className="mb-5 flex w-fit flex-wrap items-center gap-1 rounded-lg border border-border/70 bg-muted/40 p-1" role="tablist" aria-label="BNG management sections"><button type="button" role="tab" aria-selected={activeTab === 'ssh'} onClick={() => selectTab('ssh')} className={`rounded-md px-4 py-2 text-xs font-medium transition-colors ${activeTab === 'ssh' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>SSH session</button>{driver.serviceTabs.map(tab => <button type="button" role="tab" aria-selected={activeTab === tab.value} key={tab.value} onClick={() => selectTab(tab.value)} className={`rounded-md px-4 py-2 text-xs font-medium transition-colors ${activeTab === tab.value ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>{tab.label}</button>)}<button type="button" role="tab" aria-selected={activeTab === 'bng_settings'} onClick={() => selectTab('bng_settings')} className={`rounded-md px-4 py-2 text-xs font-medium transition-colors ${activeTab === 'bng_settings' ? 'bg-background text-foreground shadow-sm' : 'text-muted-foreground hover:text-foreground'}`}>BNG Settings</button></div>{tabContent}</CardContent></Card>{cgnatModalOpen && <CgnatModal values={cgnatForm} saving={cgnatSaving} onChange={(key, value) => setCgnatForm(current => ({ ...current, [key]: value }))} onClose={() => setCgnatModalOpen(false)} onPreview={previewCgnat} onSubmit={() => void saveCgnat()} />}{forwardingModalOpen && <ForwardingModal values={forwardingForm} rows={forwardingRows} loading={forwardingLoading} saving={forwardingSaving} onChange={(key, value) => setForwardingForm(current => ({ ...current, [key]: value }))} onRemove={rule => void removeForwarding(rule)} onClose={() => setForwardingModalOpen(false)} onPreview={previewForwarding} onSubmit={() => void saveForwarding()} />}{radiusModalOpen && <RadiusModal values={radiusForm} editing={Boolean(editingRadius)} saving={radiusSaving} testing={radiusTesting} onChange={(key, value) => setRadiusForm(current => ({ ...current, [key]: value }))} onClose={() => setRadiusModalOpen(false)} onTest={() => void testRadiusDatabase()} onSubmit={() => void saveRadius()} />}{vlanSyncModalOpen && <VlanSyncModal values={vlanSyncForm} options={oltOptions} saving={vlanSyncSaving} onChange={(key, value) => setVlanSyncForm(current => ({ ...current, [key]: value }))} onClose={() => setVlanSyncModalOpen(false)} onSubmit={() => void saveVlanSync()} />}{preview !== null && <div className="billing-modal-backdrop fixed inset-0 z-50 grid place-items-center overflow-y-auto p-4" role="dialog" aria-modal="true"><Card className="billing-modal-card relative w-full max-w-5xl shadow-2xl"><CardHeader className="border-b pr-14"><CardTitle>{previewTitle}</CardTitle><CardDescription>Review the commands or configuration before sending it to the BNG server.</CardDescription><Button type="button" variant="ghost" size="icon-sm" className="absolute right-4 top-4" onClick={() => setPreview(null)} aria-label="Close preview"><XIcon /></Button></CardHeader><CardContent className="pt-5"><pre className="max-h-[60vh] overflow-auto border border-border/70 bg-muted/30 p-4 font-mono text-xs leading-5">{preview}</pre><div className="mt-4 flex justify-end"><Button type="button" onClick={() => setPreview(null)}>Close preview</Button></div></CardContent></Card></div>}</div>
}

function BngSettings({ rows, loading, onAdd, onToggle, onRemove }: { rows: VlanSync[]; loading: boolean; onAdd: () => void; onToggle: (sync: VlanSync) => void; onRemove: (sync: VlanSync) => void }) {
  return <section className="border border-border/70 bg-background/40 p-4">
    <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
      <div><h2 className="text-sm font-semibold">VLAN sync</h2><p className="mt-1 text-xs text-muted-foreground">Choose which OLT VLAN records should create interfaces on this BNG.</p></div>
      <Button type="button" onClick={onAdd}><PlusIcon data-icon="inline-start" />Add VLAN sync</Button>
    </div>
    <div className="overflow-x-auto border border-border/60">
      <table className="w-full min-w-[620px] text-left text-xs">
        <thead className="bg-muted/30 text-muted-foreground"><tr><th className="px-3 py-3 font-normal">OLT</th><th className="px-3 py-3 font-normal">VLAN type</th><th className="px-3 py-3 font-normal">Sync VLAN</th><th className="px-3 py-3 font-normal">Interfaces</th><th className="px-3 py-3 text-right font-normal">Actions</th></tr></thead>
        <tbody>{loading ? <tr><td colSpan={5} className="h-28 text-center text-muted-foreground">Loading VLAN sync settings...</td></tr> : rows.length ? rows.map(sync => <tr className="border-t border-border/60" key={sync.public_id}><td className="px-3 py-3 font-medium">{sync.olt?.name || 'Unknown OLT'}</td><td className="px-3 py-3">{sync.vlan_mode === 'qinq' ? 'QinQ VLAN' : 'Normal VLAN'}</td><td className="px-3 py-3"><button type="button" role="switch" aria-checked={sync.enabled} aria-label={`Sync VLAN for ${sync.olt?.name || 'OLT'}`} onClick={() => onToggle(sync)} className={`relative inline-flex h-5 w-9 items-center rounded-full border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/50 focus-visible:ring-offset-2 ${sync.enabled ? 'border-emerald-600 bg-emerald-600' : 'border-border bg-muted'}`}><span className={`inline-block h-3.5 w-3.5 rounded-full bg-background shadow-sm transition-transform ${sync.enabled ? 'translate-x-4' : 'translate-x-0.5'}`} /></button><span className="ml-2 text-xs text-muted-foreground">{sync.enabled ? 'Enabled' : 'Disabled'}</span></td><td className="px-3 py-3 font-mono text-[11px]">{sync.interfaces?.length ? sync.interfaces.map(item => item.interface_name).join(', ') : 'Waiting for VLAN records'}</td><td className="px-3 py-3 text-right"><Button type="button" variant="ghost" size="icon-sm" onClick={() => onRemove(sync)} aria-label={`Remove VLAN sync for ${sync.olt?.name || 'OLT'}`}><TrashIcon /></Button></td></tr>) : <tr><td colSpan={5} className="h-28 text-center text-muted-foreground">No VLAN sync rules yet. Add an OLT to synchronize its VLAN interfaces.</td></tr>}</tbody>
      </table>
    </div>
  </section>
}

function VlanSyncModal({ values, options, saving, onChange, onClose, onSubmit }: { values: VlanSyncForm; options: OltOption[]; saving: boolean; onChange: (key: keyof VlanSyncForm, value: string | boolean) => void; onClose: () => void; onSubmit: () => void }) {
  return <div className="billing-modal-backdrop fixed inset-0 z-50 grid place-items-center overflow-y-auto p-4" role="dialog" aria-modal="true" aria-labelledby="vlan-sync-title">
    <Card className="billing-modal-card relative w-full max-w-xl shadow-2xl">
      <CardHeader className="billing-modal-header border-b pr-14"><p className="billing-modal-eyebrow">BNG settings</p><CardTitle id="vlan-sync-title">Add VLAN sync</CardTitle><CardDescription>Choose the OLT and VLAN convention this BNG should follow.</CardDescription><Button type="button" variant="ghost" size="icon-sm" className="billing-modal-close absolute right-4 top-4" onClick={onClose} aria-label="Close VLAN sync form"><XIcon /></Button></CardHeader>
      <CardContent className="billing-modal-content pt-5"><form className="grid gap-4" onSubmit={event => { event.preventDefault(); onSubmit() }}>
        <div className="flex items-center justify-between gap-4 border border-border/70 bg-muted/20 px-3 py-2.5"><div><p className="text-xs font-semibold">Sync VLAN</p><p className="mt-0.5 text-[11px] text-muted-foreground">Create and remove matching interfaces automatically.</p></div><button type="button" role="switch" aria-checked={values.enabled} aria-label="Sync VLAN" onClick={() => onChange('enabled', !values.enabled)} className={`relative inline-flex h-6 w-11 shrink-0 items-center rounded-full border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/50 focus-visible:ring-offset-2 ${values.enabled ? 'border-emerald-600 bg-emerald-600' : 'border-slate-300 bg-slate-200'}`}><span className={`inline-block h-4 w-4 rounded-full bg-white shadow-sm transition-transform ${values.enabled ? 'translate-x-6' : 'translate-x-1'}`} /></button></div>
        <label className="grid gap-1.5 text-xs font-medium" htmlFor="vlan-sync-olt">OLT<select id="vlan-sync-olt" className="h-9 w-full border border-input bg-background px-2.5 text-xs outline-none focus-visible:border-ring focus-visible:ring-1 focus-visible:ring-ring/50" value={values.olt_id} onChange={event => onChange('olt_id', event.target.value)} required><option value="">Select OLT</option>{options.map(olt => <option key={olt.public_id} value={olt.public_id}>{olt.name} · {olt.vendor}</option>)}</select></label>
        {values.enabled && <label className="grid gap-1.5 text-xs font-medium" htmlFor="vlan-sync-mode">VLAN type<select id="vlan-sync-mode" className="h-9 w-full border border-input bg-background px-2.5 text-xs outline-none focus-visible:border-ring focus-visible:ring-1 focus-visible:ring-ring/50" value={values.vlan_mode} onChange={event => onChange('vlan_mode', event.target.value)} required><option value="normal">Normal VLAN</option><option value="qinq">QinQ VLAN</option></select></label>}
        <div className="flex justify-end gap-2 border-t border-border/70 pt-4"><Button type="button" variant="outline" onClick={onClose}>Cancel</Button><Button type="submit" disabled={saving || !values.olt_id}>{saving ? <><CircleNotchIcon className="animate-spin" data-icon="inline-start" />Saving...</> : 'Add VLAN sync'}</Button></div>
      </form></CardContent>
    </Card>
  </div>
}

function AccelPppForm({ token, publicId, values, radiusServers, loading, saving, onChange, onRadiusSelect, onPreview, onSave }: { token: string; publicId: string; values: AccelValues; radiusServers: RadiusServer[]; loading: boolean; saving: boolean; onChange: (key: keyof AccelValues, value: string) => void; onRadiusSelect: (server?: RadiusServer) => void; onPreview: () => void; onSave: () => void }) {
  const [section, setSection] = useState<'pppoe' | 'ip_pool' | 'radius' | 'dns' | 'speed_boost'>('pppoe')
...
function RadiusServers({ servers, loading, onAdd, onEdit, onRemove }: { servers: RadiusServer[]; loading: boolean; onAdd: () => void; onEdit: (server: RadiusServer) => void; onRemove: (server: RadiusServer) => void }) {
  return <section className="border border-border/70 bg-background/40 p-4">
    ...
    <div className="overflow-x-auto border border-border/60"><table className="w-full min-w-[820px] text-left text-xs"><thead className="bg-muted/30 text-muted-foreground"><tr><th className="px-3 py-3 font-normal">Server name</th><th className="px-3 py-3 font-normal">Server address</th><th className="px-3 py-3 font-normal">Auth port</th><th className="px-3 py-3 font-normal">Accounting port</th><th className="px-3 py-3 font-normal">Subscriber sync</th><th className="px-3 py-3 font-normal">Status</th><th className="px-3 py-3 text-right font-normal">Actions</th></tr></thead><tbody>{loading ? <tr><td colSpan={7} className="h-32 text-center text-muted-foreground">Loading RADIUS servers...</td></tr> : servers.length ? servers.map(server => <tr className="border-t border-border/60" key={server.public_id}><td className="px-3 py-3 font-medium">{server.name}</td><td className="px-3 py-3 font-mono text-[11px]">{server.server_address}</td><td className="px-3 py-3">{server.auth_port}</td><td className="px-3 py-3">{server.accounting_port}</td><td className="px-3 py-3"><Badge variant="outline" className={server.sync_subscribers ? 'border-emerald-300 bg-emerald-50 text-emerald-700' : 'text-muted-foreground'}>{server.sync_subscribers ? 'Enabled' : 'Off'}</Badge></td><td className="px-3 py-3">{server.status || 'draft'}</td><td className="px-3 py-3 text-right"><Button type="button" variant="ghost" size="icon-sm" onClick={() => onEdit(server)} aria-label={`Edit ${server.name}`}><PencilSimpleIcon /></Button><Button type="button" variant="ghost" size="icon-sm" onClick={() => onRemove(server)} aria-label={`Remove ${server.name}`}><TrashIcon /></Button></td></tr>) : <tr><td colSpan={7} className="h-36 text-center text-muted-foreground">No RADIUS servers yet. Add a server to configure BNG authentication.</td></tr>}</tbody></table></div>
  </section>
}

function RadiusModal({ values, editing, saving, testing, onChange, onClose, onTest, onSubmit }: { values: RadiusForm; editing: boolean; saving: boolean; testing: boolean; onChange: (key: keyof RadiusForm, value: string | boolean) => void; onClose: () => void; onTest: () => void; onSubmit: () => void }) {
  const input = (key: Exclude<keyof RadiusForm, 'sync_subscribers'>, label: string, placeholder: string, type = 'text') => <label className="grid gap-1.5 text-xs font-medium" htmlFor={`radius-${key}`}>{label}<Input id={`radius-${key}`} type={type} value={values[key]} placeholder={placeholder} required={editing ? !['secret', 'database_password', 'notes'].includes(key) : key !== 'notes'} onChange={event => onChange(key, event.target.value)} /></label>
  return <div className="billing-modal-backdrop fixed inset-0 z-50 grid place-items-center overflow-y-auto p-4" role="dialog" aria-modal="true" aria-labelledby="radius-title"><Card className="billing-modal-card relative w-full max-w-4xl shadow-2xl"><CardHeader className="billing-modal-header border-b pr-14"><p className="billing-modal-eyebrow">{editing ? 'Update record' : 'Create record'}</p><CardTitle id="radius-title">{editing ? 'Edit RADIUS server' : 'New RADIUS server'}</CardTitle><CardDescription>Configure the authentication endpoint and its database connection.</CardDescription><Button type="button" variant="ghost" size="icon-sm" className="billing-modal-close absolute right-4 top-4" onClick={onClose} aria-label="Close RADIUS form"><XIcon /></Button></CardHeader><CardContent className="billing-modal-content pt-5"><form className="grid gap-4" onSubmit={event => { event.preventDefault(); onSubmit() }}><div className="grid gap-4 sm:grid-cols-2">{input('name', 'Server name', 'Primary RADIUS')}{input('server_address', 'Server address', '10.0.10.192')}{input('secret', 'Shared secret', 'Leave blank to keep current', 'password')}{input('database_name', 'Database name', 'radius')}{input('database_username', 'Database username', 'radius')}{input('database_password', 'Database password', 'Leave blank to keep current', 'password')}{input('auth_port', 'Authentication port', '1812', 'number')}{input('accounting_port', 'Accounting port', '1813', 'number')}<div className="sm:col-span-2"><div className="flex items-center justify-between gap-4 border border-border/70 bg-muted/20 px-4 py-3"><div><p className="text-xs font-semibold text-foreground">Sync Subscribers</p><p className="mt-1 max-w-xl text-[11px] leading-4 text-muted-foreground">Keep the RADIUS <span className="font-mono">isp_subscribers</span> table aligned with subscribers that have a PPP username.</p></div><button type="button" role="switch" aria-checked={values.sync_subscribers} aria-label="Sync Subscribers" onClick={() => onChange('sync_subscribers', !values.sync_subscribers)} className={`relative inline-flex h-6 w-11 shrink-0 items-center rounded-full border transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500/50 focus-visible:ring-offset-2 ${values.sync_subscribers ? 'border-emerald-600 bg-emerald-600' : 'border-slate-300 bg-slate-200'}`}><span className={`inline-block h-4 w-4 rounded-full bg-white shadow-sm transition-transform ${values.sync_subscribers ? 'translate-x-6' : 'translate-x-1'}`} /></button></div>{values.sync_subscribers && <p className="mt-2 text-[11px] text-emerald-700">Enabled: saving this server will backfill current PPP subscribers, then keep future changes in sync.</p>}</div><label className="grid gap-1.5 text-xs font-medium sm:col-span-2" htmlFor="radius-notes">Notes<textarea id="radius-notes" className="min-h-20 w-full border border-input bg-background px-2.5 py-2 text-xs outline-none focus-visible:border-ring focus-visible:ring-1 focus-visible:ring-ring/50" value={values.notes} placeholder="Optional context for this server" onChange={event => onChange('notes', event.target.value)} /></label></div><div className="flex flex-wrap justify-end gap-2 border-t border-border/70 pt-4"><Button type="button" variant="outline" onClick={onClose}>Cancel</Button><Button type="button" variant="outline" disabled={testing || saving} onClick={onTest}>{testing ? <><CircleNotchIcon className="animate-spin" data-icon="inline-start" />Testing...</> : 'Test database'}</Button><Button type="submit" disabled={saving || testing}>{saving ? <><CircleNotchIcon className="animate-spin" data-icon="inline-start" />Saving...</> : <><FloppyDiskIcon data-icon="inline-start" />{editing ? 'Save changes' : 'Add Radius'}</>}</Button></div></form></CardContent></Card></div>
}
