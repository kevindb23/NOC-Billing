      ...
      <CardContent className="p-0">
        <div className="overflow-x-auto">
          <Table className="min-w-[980px]">
            <TableHeader><TableRow><TableHead className="w-10"><SelectAllCheckbox checked={allSelected} onChange={toggleAll} /></TableHead><TableHead>Router</TableHead><TableHead>Public ID</TableHead><TableHead>Vendor</TableHead><TableHead>Model</TableHead><TableHead>Management endpoint</TableHead><TableHead>Driver / transport</TableHead><TableHead>Status</TableHead><TableHead>Last contact</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader>
            <TableBody>
              {loading ? <TableRow><TableCell colSpan={9} className="h-32 text-center text-muted-foreground">Loading router inventory…</TableCell></TableRow> : filteredRows.length ? filteredRows.map(row => <TableRow key={row.public_id}>
                <TableCell><RowCheckbox checked={selected.has(row.public_id)} onChange={() => toggle(row.public_id)} label="Select router" /></TableCell><TableCell className="font-medium">{row.name}</TableCell>
                <TableCell className="font-mono text-[10px] text-muted-foreground">{row.public_id}</TableCell>
                <TableCell>{vendorLabel(row.vendor)}</TableCell>
                <TableCell>{row.model || 'Model not set'}</TableCell>
