...
export function RoutersPage({ token, permissions, isSuperadmin }: { token: string; permissions?: string[]; isSuperadmin?: boolean }) {
  ...
  const permanentlyDelete = async (row: Router) => {
    try {
      await notify.promise(apiRequest(`/routers/${row.public_id}?permanent=1`, { method: 'DELETE' }, token), { loading: 'Deleting router permanently…', success: 'Router permanently deleted.', error: 'Unable to permanently delete router.' })
      await load()
    } catch (exception) {
      setPageAlert({ title: 'Unable to permanently delete router', message: getErrorMessage(exception, 'Unable to permanently delete router.') })
    }
  }
