...
const configs: Record<View, { endpoint: string; title: string; eyebrow: string; description: string; columns: string[]; createFields: CrudField[]; editFields: CrudField[] }> = {
  ...
  subscriptions: { endpoint: 'subscriptions', title: 'Subscriptions', eyebrow: 'Service commitments', description: 'Assign a plan to a subscriber and manage billing dates.', columns: ['Subscriber', 'Service ID', 'Plan', 'Starts on', 'Next billing date'], createFields: [{ name: 'subscriber_id', label: 'Subscriber', required: true }, { name: 'plan_id', label: 'Plan', required: true }, { name: 'starts_on', label: 'Starts on', type: 'date', required: true }, { name: 'next_billing_date', label: 'Next billing date (automatic)', type: 'date', required: true, readOnly: true }], editFields: [{ name: 'starts_on', label: 'Starts on', type: 'date', required: true }, { name: 'next_billing_date', label: 'Next billing date (automatic)', type: 'date', required: true, readOnly: true }] },
...
function rowCells(view: View, row: Row, actions: ReactNode) {
  ...
  if (view === 'subscriptions') return <><TableCell className="font-medium">{row.service?.customer?.legal_name || '—'}</TableCell><TableCell className="font-mono text-[10px] text-muted-foreground">{row.service?.service_number || '—'}</TableCell><TableCell>{row.plan_name_snapshot || '—'}</TableCell><TableCell>{formatDate(row.starts_on)}</TableCell><TableCell>{formatDate(row.next_billing_date)}</TableCell>{actions}</>
...
export function ResourceTablePage({ session, view }: { session: Session; view: View }) {
  ...
  const { selected, toggle, toggleAll, clear, allSelected } = useTableSelection(rows, row => String(rowId(view, row)))
