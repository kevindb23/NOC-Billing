...
type Session = { token: string; is_superadmin?: boolean }
type ModalState = { mode: 'view' | 'create' | 'edit'; row?: Row } | null
type ImportRow = Record<string, string>
type RadiusSync = { ready: boolean; enabled_server_count: number; servers: Array<{ id: number; name: string; status: string }> }
...
export function SubscribersPage({ session }: { session: Session }) {
  ...
  const [radiusSync, setRadiusSync] = useState<RadiusSync | null>(null)
  const { selected, toggle, toggleAll, clear, allSelected } = useTableSelection(rows, row => row.public_id)
  const confirm = useConfirm()
  const load = useCallback(() => apiRequest<any>(`/customers?search=${encodeURIComponent(search)}`, {}, session.token).then(response => { setRows(response.data.data); setTotal(response.data.total); setRadiusSync(response.radius_sync || null) }).catch(exception => setError(getErrorMessage(exception, 'Unable to load subscribers.'))), [search, session.token])
  ...
  const revealSecret = async (fieldName: string): Promise<string> => {
    if (fieldName !== 'ppp_password' || session.is_superadmin !== true || !modal?.row?.public_id) return ''
    const response = await apiRequest<{ data: { ppp_password: string } }>(`/customers/${modal.row.public_id}/reveal-ppp-password`, { method: 'POST' }, session.token)
    return response.data.ppp_password
  }
