...
export function BngPage({ token, onManage }: Props) {
  ...
  const sessionTimers = useRef(new Set<number>())
  ...
  const refreshSession = async (row: BngRecord) => {
    try {
      ...
      if (sessionActive(nextStatus)) {
        const timer = window.setTimeout(() => {
          sessionTimers.current.delete(timer)
          void refreshSession(row)
        }, 5000)
        sessionTimers.current.add(timer)
      }
  ...
  useEffect(() => {
    void load()
    return () => {
      sessionTimers.current.forEach(timer => window.clearTimeout(timer))
      sessionTimers.current.clear()
    }
  }, [token])
