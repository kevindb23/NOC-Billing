...
Commit: c42ef6b plus the follow-up hardening commit
...
Hardening included in the follow-up: operation routes no longer depend on
`ResolveOrganization`, operation permissions are distinct with legacy fallback,
credential profile/version snapshots are persisted and verified at execution,
compatibility request IDs use the operation validator, and operation creation
and dispatch are transactional. Focused checks now pass 12 tests and 39
assertions; PHP lint and route registration checks pass.
